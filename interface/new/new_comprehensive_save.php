...
if ($alertmsg) {
  ...
  // Auto-open the printable clinic registration card (barcode of Registration No.)
  // in a popup.
  echo "window.open('$rootdir/patient_file/registration_card.php?pid=" . attr_url($pid) .
    "', '_blank', 'width=420,height=380');\n";
  // OPD flow: continue the main window straight to clinic allocation + auto-billing,
  // so reception need not re-find the patient under "Start OPD Visit".
  echo "window.location='$rootdir/modules/custom_modules/oe-module-opd/public/reception_register.php?" .
    "pid=" . attr_url($pid) . "&is_new=1';\n";
?>
</script>
