...
// Set pid from set_pid. Native only did this in review mode; the OPD results worklist
// (oe-module-opd/public/results_queue.php) opens a patient's result-entry screen with
// set_pid in normal (entry) mode, so honor it in both modes. Purely additive — native
// callers never pass set_pid outside review, so there is no behavior change for them.
...
// OPD Phase 9 — departmental + per-order billing gate on RESULT POSTING. For non-admin users,
// every order being saved must (a) belong to a department this user is allowed to result
// (lab tech → Laboratory, radiology → Radiology, doctor → Procedures/theatre) and (b) have its
// bill cleared at the cashier (per-order waterfall). Hard block; admin/super exempt. This is the
// non-bypassable backstop — the results_queue page already filters/disables what staff can reach.
if (!empty($_POST['form_submit']) && !empty($_POST['form_line']) && !AclMain::aclCheckCore('admin', 'super')) {
    require_once($GLOBALS['fileroot'] . '/interface/modules/custom_modules/oe-module-opd/src/Service/OpdOrderBillingService.php');
    $opdBilling = new \OpenEMR\Modules\OpdWorkflow\Service\OpdOrderBillingService();
    $opdOrderIds = [];
    foreach ($_POST['form_line'] as $opdLineValue) {
        $opdOid = (int) (explode(':', (string) $opdLineValue)[0] ?? 0);
        if ($opdOid > 0) {
            $opdOrderIds[$opdOid] = true;
        }
    }
    foreach (array_keys($opdOrderIds) as $opdOid) {
        if (!$opdBilling->canResultOrder($opdOid)) {
            die(xlt('You are not authorized to enter results for this department.'));
        }
        if (!$opdBilling->isOrderPaid($opdOid)) {
            die(xlt('The patient must clear the bill for this order at the cashier before results can be entered.'));
        }
    }
}
