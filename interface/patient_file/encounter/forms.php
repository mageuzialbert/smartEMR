...
<nav>
    ...
    // OPD pilot: a doctor (opd_doctor_patient.json menu role) gets view-only
    // access to encounters that are not theirs, or that are both older than a
    // day AND closed (discharged). Admins / other roles are unaffected.
    $opdDoctorReadOnly = false;
    if (!empty($GLOBALS['encounter'])) {
        $opdMenuRow = sqlQuery("SELECT patient_menu_role FROM users WHERE id = ?", [$session->get('authUserID')]);
        if (($opdMenuRow['patient_menu_role'] ?? '') === 'opd_doctor_patient.json') {
            $opdSvc = new OpenEMR\Modules\OpdWorkflow\Service\OpdEncounterService();
            $opdDoctorReadOnly = !$opdSvc->isEncounterEditableByDoctor(
                (int)$GLOBALS['encounter'],
                (int)$session->get('authUserID')
            );
        }
    }
    ...
    echo $t->render('encounter/forms/navbar.html.twig', [
        ...
        // OPD pilot: hide the "Add New Form" menu when the doctor only has
        // view-only access to this (frozen) encounter.
        'menuArray' => $opdDoctorReadOnly ? [] : $menu->getMenuData(),
    ]);
    ?>
    ...
    <div id="encounter_forms" class="container-xl">
        ...
        if (
            ...
            ($result = getFormByEncounter(
                ...
                "",
                // OPD pilot: clinical reading order — Vitals → OPD Visit Note → Orders → everything
                // else (the newpatient "Visit Summary", which carries the Dispensed Medications table,
                // sorts last = medications last). FIELD list is reversed because of DESC.
                "FIELD(formdir,'procedure_order','LBFopd_visit','vitals') DESC, form_name, date DESC"
            ))
        ) {
    ...
    HTML;
                ...
                } elseif ($opdDoctorReadOnly) {
                    // OPD pilot: doctor has view-only access to this encounter.
                    echo "<a href='#' class='btn btn-text btn-sm form-edit-button-locked' id='form-edit-button-" . attr($formdir) . "-" . attr($iter['id']) . "'><i class='fa fa-eye fa-fw'></i>&nbsp;" . xlt('View only') . "</a>";
                } else {
                    if (
                ...
                if (!$opdDoctorReadOnly && (($esign->isButtonViewable() and $is_group == 0 and $authPostCalendarCategoryWrite) or ($esign->isButtonViewable() and $is_group and AclMain::aclCheckCore("groups", "glog", false, 'write') and $authPostCalendarCategoryWrite))) {
