...
// OPD: when the order form is opened from a department item in the encounter Orders menu
// (oe-module-opd Bootstrap::filterEncounterMenu), pre-scope the new order to that department.
if (!empty($_GET['lab_id'])) {
    $row['lab_id'] = (int)$_GET['lab_id'];
}
...
if (($_POST['bn_save'] ?? null) || !empty($_POST['bn_xmit']) || !empty($_POST['bn_save_exit'])) {
    ...
    // OPD pilot: auto-bill the ordered service codes onto the encounter Fee Sheet so the
    // cashier has a charge to collect and the per-order results gate has a target. In-house
    // orders only; idempotent per code. Never let a billing hiccup block saving the order.
    try {
        require_once($GLOBALS['fileroot'] . '/interface/modules/custom_modules/oe-module-opd/src/Service/OpdOrderBillingService.php');
        (new \OpenEMR\Modules\OpdWorkflow\Service\OpdOrderBillingService())->syncOrderCharges((int)$formid);
    } catch (\Throwable $opdBillEx) {
        (new \OpenEMR\Common\Logging\SystemLogger())->errorLogCaller('OPD order auto-bill failed: ' . $opdBillEx->getMessage());
    }
                            ...
                            <table class="table table-sm proc-table proc-table-main">
                                ...
                                <tr class="bg-primary">
                                    ...
                                    <td class='diagnosis-div'>
                                        <!-- OPD: per-line free-text Order note (stored in the diagnoses column,
                                             which already displays in the results view). Replaces the diagnosis
                                             picker — the diagnosis is captured in the OPD Visit Note. -->
                                        <textarea class='form-control' rows='1'
                                            name='form_proc_type_diag[<?php echo $i; ?>]'
                                            placeholder='<?php echo xla('Order note for this item (optional)'); ?>'></textarea>
                                    </td>
                                    <td>
                            ...
                            <table class="table table-sm proc-table proc-table-main" id="procedures_item_<?php echo (string)attr($i) ?>">
                                ...
                                <tr class="bg-primary">
                                    ...
                                    <td class='diagnosis-div'>
                                        ...
                                        <textarea class='form-control' rows='1'
                                            name='form_proc_type_diag[<?php echo $i; ?>]'
                                            placeholder='<?php echo xla('Order note for this item (optional)'); ?>'><?php echo text($oprow['diagnoses'] ?? '') ?></textarea>
                                    </td>
                                    <td>
                                        <!-- MSIE innerHTML property for a TABLE element is read-only, so using a DIV here. -->
...
<?php
// OPD pilot — simplify the Procedure Order form for ordering staff: keep only Department
// (the lab/provider selector), the procedure picker (+ Add), one free-text Order note, and a
// single "Send request" button. Everything else is hidden with safe native defaults. Admin/super
// keep the full native form for configuration/debugging. (Role-scoped core edit — track on upgrade.)
if (!\OpenEMR\Common\Acl\AclMain::aclCheckCore('admin', 'super')) { ?>
    <script>
        (function () {
            function hideRow(el) { if (el) { var r = el.closest('.form-group.form-row'); if (r) { r.style.display = 'none'; } } }
            function hideEl(el) { if (el) { el.style.display = 'none'; } }
            function ready(fn) { if (document.readyState !== 'loading') { fn(); } else { document.addEventListener('DOMContentLoaded', fn); } }
            ready(function () {
                var d = document;
                // Hide whole option rows the doctor does not need.
                ['form_order_intent', 'form_performer_type', 'form_scheduled_date', 'form_order_psc',
                 'form_specimen_fasting', 'form_history_order', 'form_order_diagnosis'
                ].forEach(function (id) { hideRow(d.getElementById(id)); });

                // Hide the whole Department/Provider/Date row — the department is chosen from the
                // encounter Orders menu (pre-set via ?lab_id) and printed in the "Procedure Order
                // Details — <department>" heading below. The hidden form_lab_id select keeps its
                // preset value, so the procedure picker still filters to that department.
                hideRow(d.getElementById('provider_id'));

                // Notes are now per-procedure (the per-row "Order note" replaces Diagnosis Codes),
                // so hide the whole order-level notes row (clinical history + patient instructions).
                hideRow(d.getElementById('form_clinical_hx'));

                // Buttons: a single primary "Send request" (save + exit); hide Save-and-Continue,
                // Transmit, and the labcorp eReq button.
                hideEl(d.getElementById('bn_save'));
                hideEl(d.getElementById('bn_save_ereq'));
                hideEl(d.querySelector('.btn-transmit'));
                var send = d.getElementById('bn_save_exit');
                if (send) {
                    send.textContent = <?php echo xlj('Send request'); ?>;
                    send.classList.remove('btn-success'); send.classList.add('btn-primary');
                }
            });
        })();
    </script>
